...
@section('title', 'Tambah Badge - AKU DEV')
...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h2 class="text-3xl font-bold text-white mb-2 font-bitter">Tambah Badge</h2>
            <p class="text-slate-400">Buat pencapaian baru untuk pengguna</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.badges.store') }}">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-white placeholder-slate-400 transition-all duration-200" placeholder="Contoh: Pemula Sejati" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-300 mb-3">Syarat Level</label>
                        <input type="number" name="level_requirement" value="{{ old('level_requirement') }}" min="1" class="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-white placeholder-slate-400 transition-all duration-200" placeholder="Contoh: 5" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-300 mb-3">Hadiah Title</label>
                        <input type="text" name="reward_title" value="{{ old('reward_title') }}" class="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-white placeholder-slate-400 transition-all duration-200" placeholder="Contoh: Rookie Developer" required>
                        <p class="text-sm text-slate-400 mt-2">Title ini akan bisa dipilih user di profile mereka</p>
                    </div>
                    <div class="pt-6">
                        <button type="submit" class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold px-8 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">Simpan Badge</button>
                    </div>
                </div>
            </form>
        </div>
