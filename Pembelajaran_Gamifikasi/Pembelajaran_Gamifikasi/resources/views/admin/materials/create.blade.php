...
<head>
    ...
    <title>Tambah Materi - AKU DEV</title>
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-8">
            <div class="mb-8">
                ...
                <h2 class="text-3xl font-bold text-white mb-2 font-bitter">Tambah Materi</h2>
            ...
            <div class="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-2xl shadow-2xl p-8">
                <form method="POST" action="{{ route('admin.materials.store') }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label for="title" class="block text-sm font-semibold text-slate-300 mb-3">Title</label>
                        ...
                        <div>
                            <label for="description" class="block text-sm font-semibold text-slate-300 mb-3">Description</label>
                            <input type="text" name="description" id="description" required
                                class="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-white placeholder-slate-400 transition-all duration-200"
                                value="{{ old('description') }}" placeholder="Deskripsi singkat untuk card user">
                            @error('description')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="bg-slate-700/50 border border-slate-600 rounded-lg overflow-hidden">
                                <textarea name="content" id="content" rows="12" class="w-full bg-transparent text-white placeholder-slate-400 resize-none border-0 focus:ring-0">{{ old('content') }}</textarea>
                            </div>
                            @error('content')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                            @enderror
